<?php
namespace Ithappens\Quote\Model\Carrier;
...
const LOG = 'ithappens.log';
const EVENT = 'ithappens_dynamic_zipcode';
protected $_code = 'ithappens';
...
public function __construct(
    ...
    \Ithappens\Quote\Helper\Data $helper,
    \Ithappens\Quote\Helper\Api $api,
    \Ithappens\Quote\Model\QuoteFactory $quoteFactory,
    array $data = []
)
{
...
{
    return ['ithappens' => $this->getConfigData ('name')];
}

public function collectRates(RateRequest $request, $pickup = false)
{
    $this->logIntelipost('ithappens collectrates');
    ...
    {
        if ($this->_helper->isModuleEnabled($this->getConfigData ('marketplace_module_name')) && !strcmp ($this->_code, 'ithappens'))
        {
            return false;
        }
    }
    ...
    // Data
    $originZipcode = $request->getOriginZipcode() ? $request->getOriginZipcode() : $this->_scopeConfig->getValue ('carriers/ithappens/source_zip');
    ...
    // Default Config
    $heightAttribute = $this->_scopeConfig->getValue ('carriers/ithappens/height_attribute');
    $widthAttribute = $this->_scopeConfig->getValue ('carriers/ithappens/width_attribute');
    $lengthAttribute = $this->_scopeConfig->getValue ('carriers/ithappens/length_attribute');

    $useCategoryAttribute = $this->_scopeConfig->getValue ('carriers/ithappens/use_category_attribute');

    $weightUnit = $this->_scopeConfig->getValue ('carriers/ithappens/weight_unit') == 'gr' ? 1000 : 1;
    $weightContingency = intval ($this->_scopeConfig->getValue ('carriers/ithappens/weight_contingency')) / $weightUnit;

    $heightContingency = $this->_scopeConfig->getValue ('carriers/ithappens/height_contingency');
    $widthContingency = $this->_scopeConfig->getValue ('carriers/ithappens/width_contingency');
    $lengthContingency = $this->_scopeConfig->getValue ('carriers/ithappens/length_contingency');

    $estimateDeliveryDate = $this->_scopeConfig->getValue ('carriers/ithappens/estimate_delivery_date');

    $calendarOnlyCheckout = $this->_scopeConfig->getValue ('carriers/ithappens/calendar_only_checkout');
    $pageName = $this->_helper->getPageName();

    $breakOnError = $this->_scopeConfig->getValue ('carriers/ithappens/break_on_error');
    $valueOnZero = $this->_scopeConfig->getValue ('carriers/ithappens/value_on_zero');
    ...
    foreach ($request->getAllItems () as $item)
    {
        ...
        $productFinalPrice = 0;
        if (!strcmp ($this->_scopeConfig->getValue ('carriers/ithappens/price_config'), 'product'))
        {
    ...
    try
    {
        ...
        $response = $this->_api->quoteRequest (\Ithappens\Quote\Helper\Api::POST, \Ithappens\Quote\Helper\Api::QUOTE_BY_PRODUCT, $postData, $api_key);

        $this->logIntelipost('apiresponse');

        $ithappensQuoteId = $response ['content']['id'];
    }
    catch (\Exception $e)
    {
        $error = $this->_rateErrorFactory->create();

        $specificerrmsg = $this->_scopeConfig->getValue ('carriers/ithappens/specificerrmsg');
    ...
    foreach ($response ['content']['delivery_options'] as $child)
    {
        ...
        if(!empty($deliveryNote))
        {
            ...
            $riskareamsg = $this->_scopeConfig->getValue ('carriers/ithappens/riskareamsg');
        ...
        // Save
        $resultQuotes [] = $this->_helper->saveQuote ($this->_code, $ithappensQuoteId, $child, $postData, $volumes);
