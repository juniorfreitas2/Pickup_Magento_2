...
const RESULT_QUOTES = 'ithappens_result_quotes';
const RESULT_PICKUP = 'ithappens_result_pickup';
...
)
{
    ...
    $this->_selectedSchedulingMethod = array();
...
public function getCustomCarrierTitle($carrier = 'ithappens', $description, $estimated_delivery, $scheduled = false)
{
    if ($scheduled)
    {
        return $this->scopeConfig->getValue("carriers/ithappens/scheduled_title");
    }
    else
    {
...
public function getProductCategories ($product, $contingency = false)
{
    ...
    if (empty ($categories))
    {
        $categoriesContingency = $this->scopeConfig->getValue('carriers/ithappens/categories_contingency');

        if ($contingency && $categoriesContingency)
        {
            $collection = $this->_categoryCollectionFactory->create();
            $collection->addAttributeToSelect('name');
            $collection->addAttributeToFilter('entity_id', array('in', $categoriesContingency));
...
public function removeQuotes($carrier = '')
{
    ...
    if (strpos ($originalPathInfo, '/shipping-information') !== false
        || strpos ($originalPathInfo, '/payment-information') !== false) return false; // keep on checkout payment

    $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
    $resource = $objectManager->get('Magento\Framework\App\ResourceConnection');
    $connection = $resource->getConnection();
    $tableName = $resource->getTableName('ithappens_quote'); //gives table name with prefix
...
public function saveQuote ($carrier, $id, $method, $postData, $volumes = false)
{
    ...
    */
    $sessionId = $this->getSessionId ();
    /*
    $quoteColl = $this->_ithappensQuote->getCollection();
...
public function checkSelected()
{
    $sessionId = $this->getSessionId();
/*
    $collection = $this->_ithappensQuote->getCollection();
...
{
    $freeshippingMethod = $this->scopeConfig->getValue ('carriers/ithappens/freeshipping_method');
    $freeshippingText   = $this->scopeConfig->getValue ('carriers/ithappens/freeshipping_text');
...
public function getPageName()
{
    ...
    else
    {
        ...
        if (!strcmp ($originalPathInfo, '/ithappens_quote/product/shipping/')) $result = 'product';
...
{
    $aditionalDeliveryDate = intval ($this->scopeConfig->getValue('carriers/ithappens/additional_delivery_date'));
    ...
    if ($wday >= 6) $timestamp += (2 * 3600 * 24);

    if (!$convert) return $timestamp;
